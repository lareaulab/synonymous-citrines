...
##  and corresponding supplementary figures                      ##
###################################################################

# The protein produced per mRNA per time showed a correlation with the elongation time of its CDS (Spearman’s ρ = −0.35) 
utr_te_cor <- cor.test( endog_with_te$TE, endog_with_te$avg_score, method = "spearman" ) # average elong speed (per codon)
# p < 2.2e-16
# rho = -0.3468937 
utr_te_rho <- round( utr_te_cor$estimate, 2)

cor.test( endog_with_te$TE, endog_with_te$tot_score, method = "spearman" ) # total elong time 
# p < 2.2e-16
# rho = -0.4304322 

# as well as with the initiation score of its UTR (Spearman’s ρ = 0.17)
all_cor <- cor.test( endog_with_te$TE, endog_with_te$MeanYFP, method = "spearman" )
# p = 1.225e-06
# rho = 0.1734522
all_rho <- round( all_cor$estimate, 2 )


## plot the relationship between elong. time and TE
cairo_pdf( file.path( figdir, "elong_vs_te.pdf"), width = 2, height = 1.3, pointsize = 6.5)
par( mex = 0.65 ) # sets margin stuff
par( mar = c(7,6.5,1,6) )
par( oma = c(0,0.5,2,0) )
par( xpd = NA )
...
axis( 2 )
title( xlab = "average elongation time\n(arbitrary units)", line = 4.5 )
text( 400, 5000, labels = paste( "r =", utr_te_rho ))
dev.off()
...
# translation efficiency of genes with faster-than-median elongation correlates slightly more strongly with the UTR strength (Spearman’s rho = 0.21), 
# while for genes with slower-than-median elongation, the correlation is lower (Spearman’s rho = 0.15). 

median_elong <- median( endog_with_te$avg_score ) # rate per codon, not total
faster <- endog_with_te[ endog_with_te$avg_score < median_elong, ]
slower <- endog_with_te[ endog_with_te$avg_score >= median_elong, ]

fast_cor <- cor.test( faster$TE, faster$MeanYFP, method = "spearman")
# p-value = 4.851e-05
# rho = 0.2052466
fast_rho <- round( fast_cor$estimate, 2)

slow_cor <- cor.test( slower$TE, slower$MeanYFP, method = "spearman")
# p = 0.002972
# rho = 0.1506296
slow_rho <- round( slow_cor$estimate, 2 )

diff <- fast_cor$estimate - slow_cor$estimate




## plot the relationship between UTR and TE
citpal <- colorRamp(citcols)
# range of elong scores
mintime <- min(avg$elongation_time) / 238
maxtime <- max(avg$elongation_time) / 238
citrange <- maxtime - mintime

cairo_pdf( file.path( figdir, "init_v_te_all.pdf"), width = 2, height = 1.3, pointsize = 6.5)
par( mex = 0.65 ) # sets margin stuff
par( mar = c(7,6.5,2,6) )
par( oma = c(0,0.5,2,0) )
par( xpd = NA )
plot( endog_with_te$MeanYFP,
      endog_with_te$TE, 
      xlim = c(0, 1),
      ylim = c(0, 6000),
      pch = 20, cex = 0.4,
      col = rgb( citpal( (endog_with_te$avg_score - mintime) / citrange ), max=255 ),
      bty = "n",
      lwd = 0.75,
      ylab = "translation efficiency",
      xlab = "initiation score"
...
title( main = "all genes",
       cex.main = 1, font.main = 1, line = 2)
text( 0.2, 5500, labels = paste( "r =", all_rho ))
imagePlot( legend.only=TRUE,
           zlim = c( 0, 1 ),
           col = colorRampPalette(citcols)(64),
           smallplot = c(.85, .875, 0.4, 0.75),
           legend.lab = "avg elong time",
           axis.args = list ( at = c(0,1), lwd = 0, mgp = c(3, 0.5, 0)) )
dev.off()

cairo_pdf( file.path( figdir, "init_v_te_faster.pdf"), width = 2, height = 1.3, pointsize = 6.5)
par( mex = 0.65 ) # sets margin stuff
par( mar = c(7,6.5,2,6) )
par( oma = c(0,0.5,2,0) )
par( xpd = NA )
plot( faster$MeanYFP,
      ...
      ylim = c(0, 6000),
      pch = 20, cex = 0.4,
      col = rgb( citpal( (faster$avg_score - mintime) / citrange ), max=255 ),
      bty = "n",
      lwd = 0.75,
      ylab = "translation efficiency",
      xlab = "initiation score"
)
title( main = "faster-than-median genes", cex.main = 1, font.main = 1, line = 2)
text( 0.2, 5500, labels = paste( "r =", fast_rho ))
dev.off()

cairo_pdf( file.path( figdir, "init_v_te_slower.pdf"), width = 2, height = 1.3, pointsize = 6.5)
par( mex = 0.65 ) # sets margin stuff
par( mar = c(7,6.5,2,6) )
par( oma = c(0,0.5,2,0) )
par( xpd = NA )
plot( slower$MeanYFP,
      ...
      ylim = c(0, 6000),
      pch = 20, cex = 0.4,
      col = rgb( citpal( (slower$avg_score - mintime) / citrange ), max=255 ),
      bty = "n",
      lwd = 0.75,
      ylab = "translation efficiency",
      xlab = "initiation score"
)
title( main = "slower-than-median genes", cex.main = 1, font.main = 1, line = 2)
text( 0.2, 5500, labels = paste( "r =", slow_rho ))
dev.off()


endog_with_te <- endog_with_te[order(endog_with_te$avg_score),]

num <- 4
q <- dim(endog_with_te)[1]/num
quantile_cors <- sapply(1:num, function(i){
  lower <- floor(q * (i-1) + 1)
  upper <- floor(q * i)
  c <- cor.test( endog_with_te$MeanYFP[lower:upper], endog_with_te$TE[lower:upper], method = "spearman")
  
  return( c(c$estimate, c$p.value))
})

sig <- rep("", num)
sig[ quantile_cors[2,] < 0.05 ] <- "*"

cairo_pdf( file.path( figdir, "endog_quantiles.pdf" ), width = 1.75, height = 1.3, pointsize = 6.5 )
par( mex = 0.65 ) # sets margin stuff
par( mar = c(6,6.5,3,3) )
par( oma = c(0,0.5,1,0) )
par( xpd = NA )
plot(1:num, quantile_cors[1,], 
     pch = 19,
     cex = 0.75,
     axes = F,
     col = "darkorchid1",
     ylim = c(0, max(quantile_cors[1,])),
     xlim = c(0.75,num),
     xlab = "quantile (fastest to slowest codons)",
     ylab = "Spearman correlation,\nUTR strength vs output"
)
axis(2)
axis(1, lwd = NA, at = 1:num)
text( 1:num, max(quantile_cors[1,]) * 1.3, labels = sig, cex = 1.5 )
dev.off()
